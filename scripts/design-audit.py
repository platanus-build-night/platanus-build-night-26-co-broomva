...
# Brand marks — DELIBERATELY EMPTY, and this is a recorded loss of coverage.
#
# The mark is the rendered keel-blade (`mark.png`), not a vector. A PNG's inks
# cannot be read out of the file the way an SVG's can, so check 8 below has
# nothing to verify and brand-colour agreement with tokens.css is enforced by
# review rather than by this gate. `skills/keel/design/README.md` says the same
# thing in the Brand section; the two must not drift apart.
#
# This is stated rather than silently dropped because a gate that quietly stops
# covering something is worse than one that never covered it — under this
# repo's own definition, an empty check that still reports success is exactly
# the `not_a_check` shape the ratio excludes.
#
# Re-populating these two lists is all that is required to restore the check if
# the marks ever return to SVG; the derive-and-compare machinery below is intact
# and was left in place for that reason. The vector arc is preserved on the
# `brand/svg-ledger-marks` branch.
BRAND_SVGS: list[str] = []
TILE_SVGS: list[str] = []
DERIVED_BANNER = (
