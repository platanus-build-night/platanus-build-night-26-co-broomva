 ...
 *
 * id:          frozen-dependency-install (v2)
 * minted from: .github/workflows/update-model-settings.yml#64-install-dependencies
 * minted at:   2026-07-25T02:26:35.904Z
 * shape:
 * A dependency install command pinned to the committed lockfile (--frozen-lockfile, --locked, --immutable, --deployment, --require-hashes). Unlike a bare install the resolver refuses to refresh a stale lock and exits non-zero, so the step is a falsifiable assertion that the manifests and the lockfile in the tree agree; it says nothing about whether the code works. v1 also carried the token "npm ci", which was withdrawn in v2: it is a substring of "pnpm ci:<task>" and made the probe assert anchored on a changesets release step. npm-ci-only repos therefore fall through to the agent, which is the safe direction.
 *
...
const ALL_OF = [];
const ANY_OF = ["npm install","pnpm install","yarn install","uv sync","pip install","poetry install","bundle install","pdm install","composer install"];
const NONE_OF = [];
...
const ASSERT_ALL = [];
const ASSERT_ANY = ["--frozen","--locked","--immutable","--deployment","--require-hashes"];
const ASSERT_NONE = ["|| true","continue-on-error: true","--no-frozen"];
...
const probe = {
  ...
  version: 2,
  mintedAt: "2026-07-25T02:26:35.904Z",
  mintedFrom: ".github/workflows/update-model-settings.yml#64-install-dependencies",
  description: "A dependency install command pinned to the committed lockfile (--frozen-lockfile, --locked, --immutable, --deployment, --require-hashes). Unlike a bare install the resolver refuses to refresh a stale lock and exits non-zero, so the step is a falsifiable assertion that the manifests and the lockfile in the tree agree; it says nothing about whether the code works. v1 also carried the token \"npm ci\", which was withdrawn in v2: it is a substring of \"pnpm ci:<task>\" and made the probe assert anchored on a changesets release step. npm-ci-only repos therefore fall through to the agent, which is the safe direction.",
  match(node) {
