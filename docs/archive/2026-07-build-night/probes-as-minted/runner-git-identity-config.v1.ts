 ...
 *
 * id:          runner-git-identity-config (v1)
 * minted from: .github/workflows/update-model-settings.yml#44-configure-git
 * minted at:   2026-07-25T02:24:54.153Z
 * shape:
 * A step whose body only sets the git author identity on the runner (git config user.email / user.name), so that later steps in the same job can commit. It reads nothing from the repository and evaluates nothing about it.
 *
 * Write-boundary argument, carried verbatim from the judgment that produced it:
 *   producer:      git writing two keys into the runner's global config file
 *   actorCanWrite: null
...
const ALL_OF = ["git config"];
const ANY_OF = ["user.email","user.name"];
const NONE_OF = [];
...
const ASSERT_ALL = ["git config"];
const ASSERT_ANY = ["user.email","user.name"];
const ASSERT_NONE = ["exit 1","git diff","pytest","vitest","assert","--check","lint"];
...
const probe = {
  id: "runner-git-identity-config",
  version: 1,
  mintedAt: "2026-07-25T02:24:54.153Z",
  mintedFrom: ".github/workflows/update-model-settings.yml#44-configure-git",
  description: "A step whose body only sets the git author identity on the runner (git config user.email / user.name), so that later steps in the same job can commit. It reads nothing from the repository and evaluates nothing about it.",
  match(node) {
  ...
  assess(node) {
    ...
    return {
      class: "not_a_check",
      writeBoundary: {"producer":"git writing two keys into the runner's global config file","actorCanWrite":null,"argument":"`git config --global user.email/user.name` sets the identity later commits in this job will carry. It reads nothing from the repository and evaluates nothing about it; the only way it exits non-zero is if the runner's HOME is unwritable. The assertions in this workflow, such as they are, live in the generate and diff steps that follow."},
      evidence: [".github/workflows/update-model-settings.yml:44-48 (git config --global user.email / user.name)",".github/workflows/update-model-settings.yml:114-125 ('Create branch and push as signed commit' — the consumer of that identity)"],
      confidence: 0.93,
    };
  },
};
